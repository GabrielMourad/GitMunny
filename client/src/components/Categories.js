...
export default function Categories() {
  ...
  function calculateTotal(type) {
    return expenses
      .filter((expense) => expense.categoryName === type)
      ...
      <>
        <div class="progress-category-together progress mt-4">
          {categories.map((category) => (
            <div
              key={category.value}
              class={`progress-bar progress-category bar-total-${
                category.value
              } bg-${categoryColor[category.value]} progress-category-together`}
              role="progressbar"
              aria-valuenow="20"
              aria-valuemin="0"
              aria-valuemax="100"
        ...
        <div>
          ...
          >
            {categories.map((category) => (
              <li
                key={category.label}
                className={`d-flex category-name text-${
  ...
  if (option === 3) {
    ...
    return (
      <>
        <img
          className="sergio"
          src="https://cdn.discordapp.com/attachments/1013580066309017603/1024099213019185212/Snapchat-866511628.jpg"
        ></img>
        <div className="option-btn">
          <button className="btn btn-light" onClick={() => setOption(1)}>
            Toggle
          </button>
        </div>
      </>
  ...
  return (
    <>
        ...
        {categories.map((category) => (
          <>
            ...
            <div className="progress progress-category mt-2">
              <div
                className={`progress-bar bar-total-${category.value} bg-success`}
                role="progressbar"
                aria-valuenow="20"
                aria-valuemin="0"
                aria-valuemax="100"
      ...
      <div className="option-btn">
        <button className="btn btn-light" onClick={() => setOption(option + 1)}>
          Toggle
        </button>
      </div>
    </>
